...
public:
	...
	explicit PATTauEmbedder(const edm::ParameterSet&);
	~PATTauEmbedder(){};
...
void PATTauEmbedder::produce(edm::Event& evt, const edm::EventSetup& es)
{
...
//  }
  ...
  for (View<pat::Tau>::const_iterator it = inputTaus->begin(), ed = inputTaus->end(); it != ed; ++it) {
    out->push_back(*it);
...
DEFINE_FWK_MODULE(PATTauEmbedder);
